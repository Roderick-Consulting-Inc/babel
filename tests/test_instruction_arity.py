...
def test_tape_spec_accepts_nonzero_arity_post_v0_5_0() -> None:
    """v0.5.0 made BF-tape interpreter arity-aware; tape specs now accept arity > 0.

    Validates the schema-level change. Whether the *runtime* can interpret a
    given arity-N op is per-op (the BF-tape interpreter currently implements
    arity-1 for JUMP_UNCONDITIONAL via tokenizer operand-consumption); the
    schema validator no longer enforces arity == 0 across the family.
    """
    spec = _bf_spec(
        extra_instructions=[
    ...
    # The schema accepts the spec; tokenizer compatibility with the chosen
    # encoding is enforced at parse time, not schema time. We use
    # whitespace_separated_tokens for the spec generated by _bf_spec, but the
    # default is ASCII_PUNCTUATION — re-create with whitespace encoding so a
    # downstream test that runs this spec can do so without tokenizer error.
    assert any(i.arity == 1 for i in spec.instructions)
